...
if (!process.env.HERE_GOOGLE_API_KEY) {
	console.warn();
	console.warn('WARNING: HERE_GOOGLE_API_KEY is not set. Page translation will be disabled.');
	console.warn('See README.md for Google Cloud API key setup.');
	if (process.platform === 'darwin') {
		console.warn('On macOS, use launchctl setenv if launching via fin:// (npm run client).');
	}
	console.warn();
}
