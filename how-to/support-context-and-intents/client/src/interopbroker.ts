 ...
 * @param InteropBroker The interop broker base type to derive from.
...
export function createInteropOverride(
	...
	class InteropOverride extends InteropBroker {
		...
		public async handleFiredIntentForContext(
			...
			| undefined
		> {
			const availableIntents = await getIntentsByContext(customSettings?.appProvider, contextForIntent.type);
