...
/**
 * Initialise the workspace platform and register the Home, Store, Dock and Notifications components.
 */
async function main(): Promise<void> {
	await WorkspacePlatform.init({
		browser: {}
	});
	// Start Notifications registration before any Workspace component registration.
	// This keeps the two system-app launches independent when one asset is unavailable.
	const notificationsRegistration = Notifications.register().then(
		() => true,
		() => false
	);
	...
	await Home.register({
		...
		onUserInput: async ({ query }): Promise<HomeSearchResponse> => {
			...
			const results = allResults.filter(
				(r) => r.key.toLowerCase().includes(lowerQuery) || r.title.toLowerCase().includes(lowerQuery)
			);
			return {
				results,
				context: {
	...
	if (await notificationsRegistration) {
