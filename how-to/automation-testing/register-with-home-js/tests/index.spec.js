const { MouseButton, OpenFinHome, OpenFinNotifications, OpenFinProxy, OpenFinSystem, WebDriver, WebDriverKeys } = require("@openfin/automation-helpers");
...
describe("Register with Home", () => {
  ...
  it("Can switch to platform window", async () => {
    const switched = await WebDriver.switchToWindow("identityString", ["register-with-home", "register-with-home"]);
  ...
  it("The runtime version should be set", async () => {
    ...
    expect(version).to.equal("43.142.102.2");
  });
  ...
  it("Can check selected entry content", async () => {
    ...
    await OpenFinHome.filtersClose(true);
    await WebDriver.sleep(500);
    // Re-run search so results are visible
    ...
    expect(itemDescriptionHtml).contains("This is an example of firing and listening to intents using the interop api and seeing a code sample of how to do it.");
  });
