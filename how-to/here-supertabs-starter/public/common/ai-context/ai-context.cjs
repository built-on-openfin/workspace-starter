...
var executeScript = (fin2) => {
	...
	const _aiContext = {
		setContextChangedListener: async (listener) => {
			...
			if (!chan) {
				chan = await fin2.InterApplicationBus.Channel.connect(`ai-context-${fin2.me.identity.uuid}`, {
					payload: 'chat-view'
				}).catch((error) => {
...
if ('fin' in window) {
	...
	const shouldExecute =
		!isServiceWorker &&
		fin.me &&
		fin.me.uuid !== fin.me.name &&
		!partialDenyUrls.includes(window.location.href);
	if (shouldExecute) {
		executeScript(fin);
	}
} else {
