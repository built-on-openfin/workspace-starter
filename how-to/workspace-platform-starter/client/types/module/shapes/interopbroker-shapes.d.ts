...
export interface PlatformInteropOverride<O = unknown, H = ModuleHelpers> extends ModuleImplementation<O, H> {
     ...
     */
    getConstructorOverride(options: PlatformInteropOverrideOptions): Promise<OpenFin.ConstructorOverride<OpenFin.InteropBroker>>;
}
/**
 ...
 */
export type IntentTargetMetaData = string | {
    appId: string;
    instanceId?: string;
};
/**
...
export interface ContextProcessorEndpoint extends Omit<Endpoint, "action"> {
     ...
     */
    requestResponse(endpointDefinition: EndpointDefinition, request: ContextToProcess): Promise<ProcessedContext>;
}
/**
