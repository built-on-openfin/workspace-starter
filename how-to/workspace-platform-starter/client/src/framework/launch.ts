	...
	try {
		...
		switch (app.manifestType) {
			...
			case MANIFEST_TYPES.DesktopBrowser.id: {
				...
				logger.info(`Launched desktop browser with url: ${app.manifest} for app: ${app.appId}. As this is a desktop browser we do not have an identity to return so we are returning the appId as the uuid and name. If the url launch triggers a registration of an intent handler by the app they should connect to the platform using their defined appId.`);
				platformAppIdentities.push({
						uuid: app.appId,
						name: app.appId,
						appId: app.appId
