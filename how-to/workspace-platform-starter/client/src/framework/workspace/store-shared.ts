...
import {
	...
	type UpdateButtonConfigRequest
...
/**
 * A minimal shape for the registration returned by either store implementation, exposing the ability to
 * update an app card's buttons (used to keep the favorite buttons in sync).
 */
export interface StorefrontButtonUpdater {
	/**
	 * Update primary and secondary button configuration for an app card.
...
/**
 * Get the store provider options that were last used to build the store provider.
 * @returns The store provider options.
 */
export function getStoreProviderOptions(): StorefrontProviderOptions | undefined {
...
/**
 * Subscribe to favorite changes so that the app card buttons can be kept in sync.
 * @param registration The registration used to update the app card buttons.
 */
export function subscribeToFavoriteChanges(registration: StorefrontButtonUpdater): void {
	favChangedSubscriptionId = subscribeLifecycleEvent<FavoriteChangedLifecyclePayload>(
		...
		async (_: unknown, payload?: FavoriteChangedLifecyclePayload) => {
			if (!isEmpty(payload)) {
				await updateAppFavoriteButtons(registration, payload);
			}
		}
	);
}
 ...
 */
export function unsubscribeFromFavoriteChanges(): void {
...
/**
 * Reset the shared store state. Called when the store component is deregistered.
 */
export function resetStoreState(): void {
	storeProviderOptions = undefined;
}
 ...
 */
export function isStorefrontConfigurationValid(): boolean {
	const idList: string[] = [];
 ...
 */
async function updateAppFavoriteButtons(
	registration: StorefrontButtonUpdater,
	payload: FavoriteChangedLifecyclePayload
): Promise<void> {
