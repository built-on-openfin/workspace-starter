...
import { type EndpointFavoriteEntry, type EndpointFavoriteGetRequest, type EndpointFavoriteGetResponse, type EndpointFavoriteListRequest, type EndpointFavoriteListResponse, type EndpointFavoriteRemoveRequest, type EndpointFavoriteSetRequest, FavoriteEntry } from "workspace-platform-starter/shapes/favorite-shapes";
...
export class FavoriteLocalStorageProvider implements Endpoint<unknown> {
	 ...
	 */
	public async action(endpointDefinition: EndpointDefinition,
		request?: EndpointFavoriteSetRequest | EndpointFavoriteRemoveRequest): Promise<boolean> {
		...
		throw new Error(`This endpoint module only supports the type 'module' and being passed a request. The following endpoint type was specified: ${endpointDefinition.type} and request passed: ${!isEmpty(request)}`);
	}
	 ...
	 */
	public async requestResponse(endpointDefinition: EndpointDefinition,
		request?: EndpointFavoriteListRequest | EndpointFavoriteGetRequest): Promise<EndpointFavoriteListResponse|
		EndpointFavoriteGetResponse|undefined> {
		if(endpointDefinition.type === "module" && !isEmpty(request)) {
			if("id" in request) {
	...
	private async requestResponseFavorites(
		request: EndpointFavoriteListRequest): Promise<EndpointFavoriteListResponse> {
	...
	private async requestResponseGetFavorite(
		request: EndpointFavoriteGetRequest): Promise<EndpointFavoriteGetResponse|undefined> {
			const favorites = this.getFavorites();
			if(!isEmpty(request.id)) {
				const matchedEntry =
				favorites.find((entry) => entry.payload.id === request.id);
				return matchedEntry;
			}
			return undefined;
	}
	 ...
	 * @returns the favorite
	 */
	private async requestResponseSetFavorite(
		request: EndpointFavoriteSetRequest): Promise<boolean> {
			const favorites = this.getFavorites();
			if(!isEmpty(request.id)) {
				const existingEntryIndex =
				favorites.findIndex((entry) => entry.payload.id === request.id);
	 ...
	 */
	private async requestResponseRemoveFavorite(
		request: EndpointFavoriteRemoveRequest): Promise<boolean> {
			const favorites = this.getFavorites();
			if(!isEmpty(request.id)) {
				const existingEntryIndex =
				favorites.findIndex((entry) => entry.payload.id === request.id);
