		...
		if (this._helpers?.subscribeLifecycleEvent) {
			...
			if (this._notificationClient) {
				...
				if (this._definition?.data?.notifyOn?.appsChanged !== false) {
					const appsChangedSubscription = await this._helpers?.subscribeLifecycleEvent(
						"apps-changed",
						async () => {
				...
				if (this._definition?.data?.notifyOn?.favoriteChanged !== false) {
					const favoriteChangedSubscription = await this._helpers?.subscribeLifecycleEvent(
						"favorite-changed",
						async () => {
							const notification: NotificationOptions = {
				...
				if (this._definition?.data?.notifyOn?.pageChanged !== false) {
					const pageChangedSubscription = await this._helpers?.subscribeLifecycleEvent(
						"page-changed",
						async () => {
							const notification: NotificationOptions = {
				...
				if (this._definition?.data?.notifyOn?.themeChanged !== false) {
					const themeChangedSubscription = await this._helpers?.subscribeLifecycleEvent(
						"theme-changed",
						async () => {
							const notification: NotificationOptions = {
								title: "Theme Changed",
				...
				if (this._definition?.data?.notifyOn?.workspaceChanged !== false) {
					const workspaceChangedSubscription = await this._helpers?.subscribeLifecycleEvent(
						"workspace-changed",
						async () => {
							const notification: NotificationOptions = {
