...
- Added new experimental support for dock3. The endpoints should still work for saving and restoring the dock configuration but please test (the shapes will be different). Dock3 can be enabled in the dockProvider settings by using the new dockType setting and specifying 3. The app configuration that allowed you to specify apps that should show as root icons or menus of apps now get mapped to the content menu. Entries that called specific actions (e.g. showing a custom native menu or calling an action such as launch app or something else you have registered) show up as root icons in the dock (dock 3 favorites). The docs related to dock has also been updated: [how to customize dock](./docs/how-to-customize-dock.md) and the main manifest.fin.json has dock3 enabled by default.
