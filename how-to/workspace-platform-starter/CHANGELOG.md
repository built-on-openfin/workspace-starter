...
- Added support for toggling the Home and Store components between the original `@openfin/workspace` implementation and the new platform (VPW) implementation (`HomeVpw`/`StorefrontVpw`) provided by `@openfin/workspace-platform`. Set `homeProvider.homeType` and `storefrontProvider.storeType` to `"workspace"` (the default) or `"platform"` to choose the implementation. When the platform implementation is selected, the component reports the `@openfin/workspace-platform` client API version rather than the `@openfin/workspace` version. The `storefrontProvider.storeWindowOptions` setting can be used to pass additional window options when the platform store is selected. See [how to customize home](./docs/how-to-customize-home.md) and [how to customize store](./docs/how-to-customize-store.md).
- Renamed the dock `dockType` values from `"1"`/`"3"` to `"workspace"`/`"platform"` so that the dock uses the same naming convention as the new `homeType` and `storeType` toggles (`"workspace"` remains the default; the main [manifest.fin.json](./public/manifest.fin.json) uses `"platform"`). The related `dockProvider` settings `dock3WindowOptions` and `dock3UIConfig` have been renamed to `dockWindowOptions` and `dockUIConfig`, and the implementation files renamed to `dock-workspace.ts`/`dock-platform.ts` to align with the Home and Store components. See [how to customize dock](./docs/how-to-customize-dock.md).
