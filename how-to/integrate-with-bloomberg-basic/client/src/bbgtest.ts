...
const initDom = () => {
	...
	selectValue.addEventListener("change", function handleInputChange(event) {
		...
		logInformation(
			`action: ${selectedIntent}, type: ${fdc3Denomination}, bbg mnemonic: ${bbgMnemonic}, search value: ${intentValue}`
		);
	});
		...
		switch (this.value) {
			case "ViewChart":
				...
				selectValue.innerHTML =
					"<option value='ORCL'>Oracle Corp</option><option value='MSFT'>Microsoft</option><option value='IBM'>IBM</option>";
				document.querySelector<HTMLSelectElement>("#intentValueChoice").selectedIndex = -1;
				break;
			case "ViewContact":
				logInformation(
				...
				bbgMnemonic = "BIO";
				selectValue.innerHTML =
					"<option value='William Henry Gates'>William Henry Gates</option><option value='Larry Ellison'>Larry Ellison</option><option value='Robert Iger'>Robert Iger</option>";
				document.querySelector<HTMLSelectElement>("#intentValueChoice").selectedIndex = -1;
				break;
			case "ViewInstrument":
				logInformation(
