...
- An implementation of InteropBroker is provided in [interopbroker.ts](client/src/framework/platform/interopbroker.ts) and it overrides handleInfoForIntentsByContext, handleInfoForIntent, handleFiredIntentForContext and handleFiredIntent
- [platform.ts](client/src/framework/platform/platform.ts) uses this implementation when calling the init function from the @openfin/workspace-platform npm module.
...
- Update [apps.ts](client/src/framework/apps.ts) to support getting apps that support intents or context types.
- Update [launch.ts](client/src/framework/launch.ts) so that it can launch views and pages directly (it needs to do this as the interopbroker needs to know the ids of the views created in order to pass the intent to them).
