...
async function saveShareRequest(payload) {
	try {
		...
		logger.error(LOGGER_GROUP, "Error saving share request", error);
		await notifyOfFailure("The share request you raised could not be generated.");
	}
}
	...
	try {
		...
		if (shareEntry !== undefined && shareEntry !== null) {
			if (shareEntry.type === "page") {
				...
				logger.warn(LOGGER_GROUP, `Share entry of unknown type specified: ${shareEntry.type}`);
		...
		logger.error(LOGGER_GROUP, "There has been an error trying to load and apply the share link", error);
...
export async function register() {
	if (!shareRegistered) {
		...
		initOptionsListenerId = registerListener("shareId", async (initOptions) => {
			logger.info(LOGGER_GROUP, "Received share request");
			if (typeof initOptions.shareId === "string") {
				await loadSharedEntry(initOptions.shareId);
			} else {
				logger.warn(LOGGER_GROUP, "shareId passed but it wasn't a string");
			}
		});
	} else {
		logger.warn(LOGGER_GROUP, "Share cannot be registered more than once");
	}
}
...
export async function deregister() {
	if (shareRegistered) {
		...
		logger.warn(LOGGER_GROUP, "Share isn't registered yet so cannot be deregistered");
	}
}

export async function showShareOptions(payload: { windowIdentity: OpenFin.Identity; x: number; y: number }) {
	if (shareRegistered) {
		logger.info(LOGGER_GROUP, "Share called with payload", payload);
		...
		if (r.result === "closed") {
			logger.info(LOGGER_GROUP, "share menu dismissed");
		...
		logger.warn(LOGGER_GROUP, "Share cannot be triggered as it hasn't been registered yet");
	}
}
...
export async function share(options?: IShareCustomData) {
	if (shareRegistered) {
		if (options === undefined || options.workspaceId !== undefined) {
			logger.info(LOGGER_GROUP, "A request to share the workspace has been raised");
			await saveSharedWorkspace(options?.workspaceId);
		} else {
			logger.info(LOGGER_GROUP, "Share called with payload", options);
