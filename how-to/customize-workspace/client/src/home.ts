...
async function getResults(
  ...
  const workspaceEntries = await mapWorkspaceEntriesToSearchEntries(workspaces as any);
          ...
          if (path.length === 1) {
            ...
            if (targetValue !== undefined && targetValue !== null && Array.isArray(targetValue)) {
              if (
                ...
                targetValue.some((matchTarget) => matchTarget.toLowerCase().indexOf(query) === 0)
              ) {
                return true;
              } else {
                console.warn(
      ...
      if (!handled) {
        if (result.data.workspaceId !== undefined) {
          if (result.data.workspaceId !== undefined && result.key === "WORKSPACE-SAVE") {
            await saveWorkspace(result.data.workspaceId, result.data.workspaceTitle);
