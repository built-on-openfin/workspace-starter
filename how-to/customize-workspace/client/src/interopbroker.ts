...
export function interopOverride(
	...
	class InteropOverride extends InteropBroker {
		public async launchAppWithIntent(app: App, intent: OpenFin.Intent) {
			logger.info("InteropBroker", "Launching app with intent");
			...
			if (app.manifestType === manifestTypes.view.id || app.manifestType === manifestTypes.inlineView.id) {
				logger.info("InteropBroker", `The supporting app is a view (${app.manifestType})`);

				const identity = await launchView(app);
				if (identity === null) {
					// optional logic show a prompt to the user to let them know
					logger.warn("InteropBroker", "Unable to raise intent against view as no identity was returned");
					return null;
				}
			...
			if (app.manifestType === manifestTypes.snapshot.id) {
				logger.info("InteropBroker", "The supporting app is a view");

				const identities = await launchSnapshot(app);
				if (identities === null) {
					// optional logic show a prompt to the user to let them know
					logger.warn("InteropBroker", "Unable to raise intent against target as no identity was returned");
					return null;
				}
		...
		public async launchAppPicker(launchOptions: {
			...
			try {
				...
				logger.error("InteropBroker", "App for intent not selected/launched", launchOptions.intent);
				return null;
			}
		}
		...
		public async isActionAuthorized(
			...
			logger.info("InteropBroker", "Is action authorized", action, payload, identity);
			...
			if (availableIntents.length === 1) {
				...
				if (availableIntents[0].apps.length > 1) {
					try {
						...
						logger.error("InteropBroker", "We were returned a non existent appId to launch with the intent");
						throw new Error(NO_APPS_FOUND);
					} catch {
						logger.error("InteropBroker", "App for intent by context not selected/launched", intent);
		...
		public async handleFiredIntent(intent: OpenFin.Intent) {
			logger.info("InteropBroker", "Received request for a raised intent", intent);
			...
			if (intentApps.length === 0) {
				logger.info("InteropBroker", "No apps support this intent");
				throw new Error(NO_APPS_FOUND);
			}

			if (targetApp !== undefined && intentApps.includes(targetApp)) {
				logger.info("InteropBroker", "Assigning selected application with intent", intent);
			...
			try {
				...
				if (intentApps === undefined) {
					logger.warn("InteropBroker", "We should have a list of apps to search from");
					intentApps = [];
				}
				...
				logger.error("InteropBroker", "App for intent not selected/launched", intent);
				throw new Error(RESOLVER_TIMEOUT);
			}
		}
	}
