...
import { AppSourceConnection, Connection, ConnectionProviderOptions, SnapshotSourceConnection } from "./connection-shapes";
...
interface ConnectedAppSourceConnection {
    identity: OpenFin.Identity; connectionData: AppSourceConnection;
}
...
async function executeAction(identity: OpenFin.Identity, payload: { action: string}): Promise<boolean> {
    ...
    console.log(`Not Executing action: ${actionId} on behalf of connection: ${identity.uuid} as the action was either not provided or not listed in the supported actions definition.`);
    return false;
}
    ...
    if (initialized) {
      ...
      if (connectionOptions?.connectionId !== undefined) {
        ...
        connectionService.onConnection((identity, payload) => {
            ...
            const validatedConnection = connectionOptions
            .connections.find((entry) => entry.identity.uuid === identity.uuid);
            let isValid = false;
            ...
            if (validatedConnection !== undefined) {
                ...
                if (isValid) {
                    if (connectedClients[identity.uuid] !== undefined) {
                        isValid = false;
                        errorMessage = "This platform can only accept one connection from an external connection. The uuid is already registered.";
                    } else {
...
export async function getConnectedAppSourceClients() {
    ...
    for (const connection of availableConnections) {
        const matchedConnection = connection.connectionTypes.find((supportedConnectionType) =>
        supportedConnectionType.type === "appSource");
...
export async function getConnectedApps(): Promise<App[]> {
    ...
    for (let i = 0; i < connectedSources.length; i++) {
        const returnedApplications: App[] = await connectionService.dispatch(connectedSources[i].identity, "getApps");
...
export async function launchConnectedApp(app: App) {
    ...
    if (app.manifestType === manifestType && connectedSource !== undefined) {
        ...
        console.warn(`A request to launch app ${app.appId} was not successful. Either the manifestType is not ${manifestType}:${app.manifestType} or the connection ${app.manifest} is either not registered in the connectionProvider with appSource support or hasn't connected to this platform.`);
    }
}
...
export async function getConnectedSnapshotSourceClients() {
    ...
    for (const connection of availableConnections) {
        const matchedConnection = connection.connectionTypes.find((supportedConnectionType) =>
        supportedConnectionType.type === "snapshotSource");
