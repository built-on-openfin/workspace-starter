...
export async function decorateSnapshot(snapshot: OpenFin.Snapshot): Promise<OpenFin.Snapshot> {
    ...
    const clientSnapshots: ClientSnapshot[] = await Promise.all(sources.map(async (uuid) => {
    ...
    const validatedClientSnapshots: ClientSnapshot[] =
    clientSnapshots.filter((snapshotSource) => snapshotSource !== null);

    const decoratedSnapshot: OpenFin.Snapshot = Object.assign(snapshot, { clientSnapshots: validatedClientSnapshots });
...
export async function applyClientSnapshot(snapshot) {
    ...
    await Promise.all(sources.map(async (uuid) => {
            const clientSnapshot: ClientSnapshot = snapshot.clientSnapshots
            .find((s: ClientSnapshot) => s.identity.uuid === uuid);
            if (clientSnapshot) {
                try {
