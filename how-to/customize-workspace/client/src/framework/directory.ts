...
async function getApps(results: PlatformApp[] | FDC3ResponseTwoPointZero | FDC3ResponseOnePointTwo):
Promise<PlatformApp[]> {
    if(Array.isArray(results)) {
        ...
        switch(version) {
            case "1.2": {
                const mappedApps: PlatformApp[] =
                mapFDC3OnePointTwoToPlatformApp((results as FDC3ResponseOnePointTwo).applications);
                return mappedApps;
            }
            case "2.0": {
                const mappedApps: PlatformApp[] =
                mapFDC3TwoPointZeroToPlatformApp((results as FDC3ResponseTwoPointZero).applications);
                return mappedApps;
            }
            default: {
...
async function getResults(directory: DirectoryEndpoint): Promise<PlatformApp[]> {
    if(directory.endpointId !== undefined) {
        ...
        .requestResponse<never, PlatformApp[]|FDC3ResponseTwoPointZero|FDC3ResponseOnePointTwo>(directory.endpointId);
        ...
        const outputResults = await endpoints
        .requestResponse<unknown, PlatformApp[] | FDC3ResponseTwoPointZero | FDC3ResponseOnePointTwo>(
            directory.map.outputId,
            inputResults
        );
...
export function addDirectory(endpoint: DirectoryEndpoint): boolean {
    if(endpoint?.id === undefined) {
        logger.error("A directory entry didn't specify an id and it needs an id to be registered. It cannot be added as a directory endpoint.", endpoint);
        return false;
    }

    if(directories.length !== 0 && (directories.findIndex((entry) => entry.id === endpoint.id) !== -1)) {
        logger.error(`A directory entry with this id: ${endpoint.id} already exists so this passed endpoint cannot be added.`, endpoint);
        return false;
    }
    ...
    if(endpoint?.endpointId !== undefined && !endpoints.hasEndpoint(endpoint.endpointId)) {
        logger.error(`A directory entry specifies an endpoint that does not exist: ${endpoint.endpointId} and it can therefore not be added as a directory endpoint. Id: ${endpoint.id}`, endpoint);
        return false;
    } else if(endpoint?.map !== undefined) {
        if(!endpoints.hasEndpoint(endpoint.map.inputId)) {
            logger.error(`A directory entry specifies an endpoint map input id that does not exist: ${endpoint.map.inputId} and it can therefore not be added as a directory endpoint. Id: ${endpoint.id}`, endpoint);
            return false;
        }
        if(!endpoints.hasEndpoint(endpoint.map.outputId)) {
            logger.error(`A directory entry specifies an endpoint map output id that does not exist: ${endpoint.map.outputId} and it can therefore not be added as a directory endpoint. Id: ${endpoint.id}`, endpoint);
            return false;
        }
    } else if(endpoint?.url?.path !== undefined && endpoint.url.path !== null && (endpoint.url.path.trim().length === 0 || !endpoint.url.path.startsWith("http"))) {
        logger.error(`A directory entry specifies an endpoint url path that is either not specified or is in the wrong format: ${endpoint?.url?.path} and it can therefore not be added as a directory endpoint. Id: ${endpoint.id}`, endpoint);
        return false;
    }
