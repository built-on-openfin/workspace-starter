import { connect, ConnectionError, enableLogging, SalesforceConnection, SalesforceRestApiSearchResponse, SalesforceRestApiSObject } from "@openfin/salesforce";
import {
    Action, CLIDispatchedSearchResult, CLIFilter,
    CLIFilterOptionType, CLISearchListenerResponse, CLISearchResponse, CLISearchResult, CLISearchResultContact,
    CLISearchResultList, CLISearchResultPlain,
    CLISearchResultSimpleText, CLITemplate, Home
...
export async function openConnection(settings?: SalesforceSettings): Promise<SalesforceConnection> {
    if (!sfConn && settings) {
        enableLogging();
        sfConn = await connect(settings.orgUrl, settings.consumerKey, settings.isSandbox);
    }
...
export async function salesForceItemSelection(
    ...
    lastResponse?: CLISearchListenerResponse): Promise<boolean> {
    ...
    if (result.key === NOT_CONNECTED_SEARCH_RESULT_KEY) {
        enableLogging();
        sfConn = await connect(integration?.data?.orgUrl, integration?.data?.consumerKey, integration?.data?.isSandbox);

        let results = await salesForceGetSearchResults(integration, result.data?.query, result.data?.filters);
        if (lastResponse) {
