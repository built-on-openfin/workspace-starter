...
10. Launch a browser window with pinned pages (three platform pins, three user pins, and one regular tab). Requires HERE Core UI workspace **45.2.6+** (see [dos.json](public/common/dos.json) or run `npm run dos` to pin locally).
11. Launch a browser window that customizes **view tab headers** (per-view icon, background/font colors, content-based tab width, and snapshot-safe tab controls). Requires HERE Core UI workspace **45.2.6+**.
...
2. Optional (if you wish to pin the version of HERE Core UI to version 45.2.6 and you are on Windows) - Set Windows registry key for [Desktop Owner Settings](https://resources.here.io/docs/core/manage/desktops/dos/).
