...
10. Launch a browser window with pinned pages (three platform pins, three user pins, and one regular tab). Requires HERE Core UI workspace **45.2.0+** (see [dos.json](public/common/dos.json) or run `npm run dos` to pin locally).
...
2. Optional (if you wish to pin the version of HERE Core UI to version 45.2.0 and you are on Windows) - Set Windows registry key for [Desktop Owner Settings](https://resources.here.io/docs/core/manage/desktops/dos/).
